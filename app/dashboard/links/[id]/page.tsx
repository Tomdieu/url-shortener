...
import {getLinksFromId} from "@/lib/getLinksFromId";
...
const id = params.id;
  const linkDetail = await getLinksFromId(parseInt(id));
...
const LinkDetail =async ({params}: Props) => {
  ...
  const data = await getLinkChartDetail(linkDetail?.short as string ,"day");

  const updateUrlAction = async (form:FormData) =>{
    'use server'
    const parse = linkSchema.safeParse({
      id:form.get('original')
    })
